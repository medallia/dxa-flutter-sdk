...
import com.medallia.dxa.common.internal.models.SamplingFlutter
...
class MedalliaDxaPlugin : FlutterPlugin, Messages.MedalliaDxaNativeApi {
    ...
    override fun enableSessionForAnalytics(msg: Boolean) {
        msg.let {
            medalliaDxa.enableSessionForAnalytics(it)
        }
    }

    override fun enableSessionForRecording(msg: Boolean) {
        msg.let {
            medalliaDxa.enableSessionForRecording(it)
        }
    }
    ...
    private fun startCollectingSamplingData(){
        binderScope.launch{
            medalliaDxa.getSampling().collect{ samplingData: SamplingFlutter ->
                log(
                        message = "new sampling data from native. $samplingData",
                        level = Log.VERBOSE
                )
                val jsonObject = JSONObject()
                jsonObject.put("stopTrackingDueToSampling", samplingData.stopTrackingDueToSampling)
                jsonObject.put("stopRecordingDueToSampling", samplingData.stopRecordingDueToSampling)

                val jsonName = JSONObject()
                jsonName.put("sampling_data", jsonObject)
                val json = jsonName.toString()
